...
<Grid>
    ...
    <!--End Header Section-->
        ...
        <StackPanel Orientation="Vertical" Width="auto" Margin="8">
            <StackPanel Orientation="Horizontal">
                <CheckBox Content="Microsoft Visual C++ 2013 (x64) Redistributable" Tag="DriRuntimesvers" IsChecked="false" FontWeight="Bold" HorizontalAlignment="Center" VerticalAlignment="Center"/>
...
try {
    ...
    # Define registry path for personalized settings
    $sync.registryPath = "HKCU:\Software\itt.emadadel"
    ...
    $sync.Langusege = (Get-ItemProperty -Path $sync.registryPath -Name "locales").locales

    # Check for language usage and set DataContext accordingly
    switch ($sync.Langusege) {
        "en" { $sync["window"].DataContext = $sync.database.locales.en }
        "ar" { $sync["window"].DataContext = $sync.database.locales.ar }
        default {
            $sync["window"].DataContext = $sync.database.locales.en
            Set-ItemProperty -Path $sync.registryPath -Name "locales" -Value "en" -Force
        }
    }

    # Retrieve Windows theme setting
    ...
    if ($sync.isDarkMode -eq "true") {
        ...
        switch ($AppsTheme) {
            "0" { $sync['window'].Resources.MergedDictionaries.Add($sync['window'].FindResource("Dark")) }
            "1" { $sync['window'].Resources.MergedDictionaries.Add($sync['window'].FindResource("Light")) }
        }
    }
    ...
    if ($error[0].Exception.Message -like "*button*") {
        Write-Warning "Ensure your <button in the `$inputXML does NOT have a Click=ButtonClick property. PS can't handle this"
    }
} catch {
    Write-Host "Unable to load Windows.Markup.XamlReader. Double-check syntax and ensure .NET is installed."
}

# Capture controls from the XAML
$sync.AppsListView = $sync['window'].FindName("appslist")
$sync.TweaksListView = $sync['window'].FindName("tweakslist")
$sync.currentList = $sync['window'].FindName("currentList")

# Capture buttons and other UI elements
