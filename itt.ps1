...
function Search{
    ...
    $filter = $sync['UI'].FindName('searchInput').Text.ToLower()
    $collectionView = [System.Windows.Data.CollectionViewSource]::GetDefaultView($sync['UI'].FindName('list').Items)
...
function Get-SelectedApps {
    ...
    foreach ($item in $sync['UI'].FindName('list').Items)
    {
        if ($item.IsChecked)
        {
...
function Get-SelectedTweeaks {
    ...
    foreach ($item in $sync['UI'].FindName('tweaks').Items)
    {
        if ($item.IsChecked)
        {
...
function Switch-ToDarkMode {
    try {
        ...
        $theme = $sync['UI'].FindResource("DarkTheme")
...
function Switch-ToLightMode {
    try {
        ...
        $theme = $sync['UI'].FindResource("LightTheme")
...
function ChangeTap() {
    ...
    if($sync['UI'].FindName('apps').IsSelected)
    {
        $sync['UI'].FindName('installBtn').Visibility = "Visible"
        $sync['UI'].FindName('applyBtn').Visibility = "Hidden"
    }

    if($window.FindName('tweeks').IsSelected)
    {
        $sync['UI'].FindName('applyBtn').Visibility = "Visible"
        $sync['UI'].FindName('installBtn').Visibility = "Hidden"
    }
}
...
$sync.list = $sync['UI'].FindName("list")
$sync.des = $sync['UI'].FindName("description")
$sync.q = $sync['UI'].FindName("quotes")

# Making the itemLink control visible
$sync['UI'].FindName('itemLink').Visibility = "Visible"
...
function UpdateDescriptionAndLink {
    ...
    foreach ($app in $sync.configs.applications) {
        if ($app.name -eq $selectedAppName) {
            ...
            $sync['UI'].FindName("description").Text = $app.description
            # Update the link text block with the selected application's official website link
            $sync['UI'].FindName('itemLink').Text = "$($app.name) official website"
            break
        }
    }
}
...
# Add event handlers
$sync['UI'].FindName("apps").add_Loaded({
    # Add a selection changed event handler to the list control
    $sync['UI'].FindName('list').Add_SelectionChanged({
        UpdateDescriptionAndLink
    })

    # Add a mouse left button down event handler to the itemLink control
    $sync['UI'].FindName('itemLink').add_MouseLeftButtonDown({
        OpenOfficialWebsite
    })
})


$sync['UI'].FindName("apps").add_LostFocus({
...
$sync.tweaks = $sync['UI'].FindName("tweaks")

# Add loaded event handler
$sync['UI'].FindName("tweeks").add_Loaded({
    ...
    $sync.tweaks.Add_SelectionChanged({
        ...
        foreach ($data in $sync.configs.tweaks) {
            if ($data.name -eq $selectedItem) {
                $sync['UI'].FindName('description').Text = $data.description
                $sync['UI'].FindName('itemLink').Visibility = if ($data.repo -ne "null") { "Visible" } else { "Hidden" }
                $sync['UI'].FindName('itemLink').Text = "Github repository"
                break
            }
        }
    })
...
$sync['UI'].FindName("tweeks").add_LostFocus({
...
# Buttons
$sync['UI'].FindName('taps').add_SelectionChanged({ChangeTap})


$sync['UI'].FindName('installBtn').add_click({Invoke-Install})

$sync['UI'].FindName('applyBtn').add_click({ApplyTweaks})

$sync['UI'].FindName('searchInput').add_TextChanged({Search})

$sync['UI'].FindName('searchInput').add_GotFocus({ClearFilter})

$sync['UI'].FindName('about').add_MouseLeftButtonDown({About})

$sync['UI'].FindName('themeText').add_click({Toggle-Theme})

# Catgoray bar buttons
$sync['UI'].FindName('all').add_click({ClearFilter})
